 ...
 */
class BrandAttributeSetThumbnailResolver implements ResolverInterface
{
    /**
     * @var BrandFactory
     */
    ...
    {
        if (isset($value['thumbnail']) && isset($value['brand_id'])
            && $value['brand_id'] && $value['thumbnail']) {
            $brand = $this->brandFactory->create()->load($value['brand_id']);
            return $brand->getThumbnailUrl();
        } else {
            return "";
        }
    }
